...
public class RedBlackTree<T extends Comparable<T>> implements java.io.Serializable {
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (parent.leftChild != child && parent.rightChild != child) {
            throw new IllegalArgumentException("Sorry, the provided child is not related to the provided parent.");
        }
    ...
    /**
     * It takes a reference to a newly added red node as its only parameter. Note
     * that this method may also be called recursively, in which case the input
     * parameter may reference a different red node in the tree that potentially has
     * a red parent node. The job of this method is to resolve red child under red
     * parent red black tree property violations that are introduced by inserting
     * new nodes into a red black tree. While doing so, all other red black tree
     * properties must also be preserved.
     *
     * @param redNode the reference to a newly added red node which might also has a
     *                red parent node.
     */
                ...
                if (redNode.parent.isLeftChild()) {
                    ...
                    if (redNode.parent.parent.rightChild == null) {
                        ...
                        if (sibling.isBlack) {
                            ...
                            redNode.isBlack = false; // set added node as red
                            redNode.parent.isBlack = true; // set its parent as black
                            redNode.parent.rightChild.isBlack = false; // set its parent's rightChild as red
                    ...
                    if (redNode.parent.parent.leftChild == null) {
                        ...
                        if (sibling.isBlack) {
                            ...
                            redNode.isBlack = false; // set added node as red
                            redNode.parent.isBlack = true; // set its parent as black
                            redNode.parent.leftChild.isBlack = false; // set its parent's leftChild as red
                        } else {// if the sibling is red
                            sibling.isBlack = true; // set sibling as black
                            redNode.parent.isBlack = true; // set parent as black
                            redNode.parent.parent.isBlack = false; // set grandparent as red
